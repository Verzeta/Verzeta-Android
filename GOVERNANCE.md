...
- **No dependency on a single closed-source service** for the project to function. The host application must remain runnable with at least one fully-local LLM provider configured (today: Ollama, or the llama.cpp engine in the Local AI edition). This Android app is a client of that host and inherits the same rule.
...
This document is part of Verzeta for Android's source tree. Material changes (governance-model evolution, jurisdictional shifts, conflict-of-interest disclosures) are made via Git commits with explanatory messages, and may also be noted in the release notes.
